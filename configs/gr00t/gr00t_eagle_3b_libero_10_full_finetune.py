...
model = dict(
    ...
    vla_head=dict(
        ...
        # Match the released LIBERO-10 checkpoint training contract. The
        # later (1.0, 1.5) schedule was never used to train that checkpoint.
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        seed=7,
        shuffle=True,
        # The historical wrapper used seed + epoch unconditionally, so the
        # released checkpoint reshuffled every epoch.
        reshuffle_each_epoch=True,
        name_mappings={
            'observation.state': ['proprio'],
            'action': ['action']
        },
        # Preserve the normalization contract used by the released LIBERO-10
        # checkpoint. It was trained with these legacy per-dimension scales;
        # dynamically recomputed population stds are sqrt(8)/sqrt(7) larger
        # for proprio/actions and therefore define a different recipe.
        ...
        datasets=dict(
            type='ParquetDataset',
            # Dataset snapshot used by the step-028548 released checkpoint.
            data_root_path=  # noqa: E251
            'datasets/libero_10_no_noops_lerobotv2.1_old',  # noqa: E501
            transforms=[
                dict(
